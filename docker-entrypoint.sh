...
if [ ! -f "/home/nonroot/.govchain/config/genesis.json" ]; then
  ...
  gosu nonroot env 'HOME=/home/nonroot' curl -sL "https://raw.githubusercontent.com/bettergovph/govchain/refs/heads/main/genesis.json" -o "$TEMP_GENESIS_PATH"
  ...
  gosu nonroot env 'HOME=/home/nonroot' govchaind init "$MONIKER" --chain-id "$CHAIN_ID" --home "/home/nonroot/.govchain"
  ...
  gosu nonroot mv "$TEMP_GENESIS_PATH" "$ACTUAL_GENESIS_PATH"
...
if [ -n "$EXTERNAL_IP" ]; then
  ...
  gosu nonroot sed -i "s/^external_address = \"\"/external_address = \"$EXTERNAL_IP:26656\"/" "/home/nonroot/.govchain/config/config.toml"
